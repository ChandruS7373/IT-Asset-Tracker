...
# CSS as a module-level constant — plain ASCII, no unicode chars, no f-string
_CSS = (
    ...
    "width:100% !important;background:transparent !important;color:#cbd5e1 !important;"
    "border:none !important;text-align:left !important;padding:9px 16px !important;"
    ...
    ".stat-card .value{font-size:2rem;font-weight:800;line-height:1;color:#1e293b;}"
    ".stat-card .label{color:#64748b;font-size:.75rem;font-weight:700;"
    ...
    ".flex-fill{flex:1 1 auto !important;}.flex-wrap{flex-wrap:wrap !important;}"
    ...
    ".small{font-size:.875rem !important;}.fs-5{font-size:1.25rem !important;}"
...
def _inject_html(html):
    """st.html() (Streamlit 1.31+) reliably renders style/link tags. Fallback to st.markdown."""
    try:
        st.html(html)
...
def status_email_html(name,status,rid,stype,asset):
    ...
    return (f"<div style='font-family:Segoe UI,sans-serif;max-width:540px;margin:auto'>"
            f"<div style='background:linear-gradient(135deg,#0f3460,#533483);padding:22px;text-align:center'>"
            f"<h2 style='color:#fff;margin:0'>Request #{rid} - {status}</h2></div>"
            f"<div style='padding:24px;border:1px solid #e2e8f0'>"
            f"<p>Hi <b>{name}</b>, your request status is now <b style='color:{c}'>{status}</b>.</p>"
            f"<p>Asset: {asset} | Issue: {stype}</p></div></div>")

def chat_email_html(sender,role,msg,rid,stype):
    rl={'admin':'Admin','technician':'Technician','user':'Employee'}.get(role,role)
    return (f"<div style='font-family:Segoe UI,sans-serif;max-width:540px;margin:auto'>"
            f"<div style='background:linear-gradient(135deg,#0f3460,#533483);padding:22px;text-align:center'>"
            f"<h2 style='color:#fff;margin:0'>New Message - Request #{rid}</h2></div>"
            f"<div style='padding:24px;border:1px solid #e2e8f0'>"
            f"<p>From <b>{sender}</b> ({rl}) on Request #{rid} ({stype}):</p>"
            f"<div style='background:#f1f5f9;border-left:4px solid #533483;padding:12px;margin:14px 0'>{msg}</div></div></div>")

# ─── Auth ──────────────────────────────────────────────────────────────────────

def page_login(logo_b64):
    _inject_html("<style>.stApp{background:linear-gradient(135deg,#0f3460 0%,#533483 100%) !important;}</style>")
    st.markdown("<br><br>", unsafe_allow_html=True)
    _, col, _ = st.columns([1, 1.2, 1])
    with col:
        logo = f'<img src="data:image/jpeg;base64,{logo_b64}" style="height:36px;margin-bottom:12px">' if logo_b64 else ""
        st.markdown(
            f'<div style="background:#fff;border-radius:20px;padding:32px;'
            f'box-shadow:0 20px 60px rgba(0,0,0,.3);text-align:center">'
            f'{logo}'
            f'<h2 style="color:#0f3460;font-weight:800;margin:8px 0 4px">IT Asset Tracker</h2>'
            f'<p style="color:#64748b;margin:0 0 20px">Qualesce Technology Solutions</p></div>',
...
def nav_btn(label, page):
    if st.sidebar.button(label, use_container_width=True, key='nav_' + page):
...
def sidebar_nav(logo_b64):
    ...
    badge_colors = {'admin': '#f59e0b;color:#1e293b', 'technician': '#22c55e;color:#fff', 'user': '#38bdf8;color:#fff'}
    logo_tag = f'<img src="data:image/jpeg;base64,{logo_b64}" style="height:28px;object-fit:contain">' if logo_b64 else "IT"
    st.sidebar.markdown(
        f'<div style="padding:14px;border-bottom:1px solid rgba(255,255,255,.1);text-align:center">'
        f'<div style="background:#fff;border-radius:10px;padding:8px 14px">{logo_tag}</div></div>'
        f'<div style="padding:12px 14px;border-bottom:1px solid rgba(255,255,255,.1)">'
        f'<div style="width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,#533483,#0f3460);'
        f'display:inline-flex;align-items:center;justify-content:center;color:#fff;font-weight:700;'
        f'margin-right:10px;vertical-align:middle">{name[0].upper()}</div>'
        f'<span style="font-weight:600;vertical-align:middle">{name}</span><br>'
        f'<span style="display:inline-block;margin-top:4px;padding:2px 10px;border-radius:20px;'
        f'font-size:.7rem;font-weight:700;background:{badge_colors.get(role,"#94a3b8;color:#fff")}">'
        f'{badges.get(role,"USER")}</span></div>',
        unsafe_allow_html=True)

    def sec(label):
        st.sidebar.markdown(
            f'<div style="padding:6px 14px 2px;font-size:.7rem;font-weight:700;'
            f'letter-spacing:.08em;color:#94a3b8;text-transform:uppercase">{label}</div>',
            unsafe_allow_html=True)

    if role == 'admin':
        sec('Dashboard'); nav_btn('📊  Dashboard', 'dashboard')
        sec('Assets'); nav_btn('💻  All Assets', 'assets'); nav_btn('👥  Employees', 'employees')
        sec('Service'); nav_btn('📥  Service Requests', 'service_requests'); nav_btn('🔧  Service History', 'services')
        sec('Management'); nav_btn('👤  Manage Users', 'users')
        sec('Settings'); nav_btn('📧  Email Config', 'email_config')
    elif role == 'technician':
        sec('My Tasks'); nav_btn('📊  Dashboard', 'dashboard')
        sec('Settings'); nav_btn('🔔  Email Preferences', 'email_prefs')
    else:
        sec('My Portal'); nav_btn('🏠  My Dashboard', 'dashboard')
        nav_btn('🎫  My Requests', 'my_requests'); nav_btn('🔧  Request Service', 'request_service')
        sec('Settings'); nav_btn('🔔  Email Preferences', 'email_prefs')
...
# Status badge HTML (self-contained, no Bootstrap needed)
def sbadge(s):
    colors = {
        'Pending':     'background:#ffc107;color:#212529',
        'Accepted':    'background:#198754;color:#fff',
        'Declined':    'background:#dc3545;color:#fff',
        'In Progress': 'background:#0d6efd;color:#fff',
        'Hold':        'background:#6c757d;color:#fff',
        'Completed':   'background:#198754;color:#fff',
    }
    st_css = colors.get(s, 'background:#6c757d;color:#fff')
    return (f'<span style="display:inline-block;padding:.3em .65em;font-size:.72rem;font-weight:700;'
            f'line-height:1;text-align:center;white-space:nowrap;vertical-align:baseline;'
            f'border-radius:.4rem;{st_css}">{s}</span>')
...
def pg_admin_dashboard():
    ...
    st.markdown('<h4 style="color:#1e293b;padding:1.5rem 2rem 0"><i class="fas fa-tachometer-alt" style="color:#94a3b8;margin-right:8px"></i>Admin Dashboard</h4>', unsafe_allow_html=True)

    def scard(key, label, icon, cls, icls):
        v = stats.get(key, 0)
        if key == 'yashwanth' and v == 0: return ''
        return (f'<div class="col-6 col-md-4 col-xl-2" style="padding:0 .4rem">'
                f'<div class="stat-card {cls}" style="margin-bottom:.8rem">'
                f'<div class="icon {icls}"><i class="{icon}"></i></div>'
                f'<div class="value">{v}</div><div class="label">{label}</div></div></div>')

    def scard2(key, label, icon, bg, ibg, vc):
        v = stats.get(key, 0)
        if key == 'yashwanth' and v == 0: return ''
        return (f'<div class="col-6 col-md-4 col-xl-2" style="padding:0 .4rem">'
                f'<div class="stat-card" style="background:{bg};margin-bottom:.8rem">'
                f'<div class="icon" style="background:{ibg}"><i class="{icon}"></i></div>'
                f'<div class="value" style="color:{vc}">{v}</div><div class="label">{label}</div></div></div>')

    cards_html = (
        '<div style="display:flex;flex-wrap:wrap;margin:0 1.6rem 1rem">'
        + scard('total',    'Total Assets',    'fas fa-laptop',     'card-primary',  'card-icon-primary')
        + scard('assigned', 'Assigned',        'fas fa-user-check', 'card-success',  'card-icon-success')
        + scard('stock',    'In Stock',        'fas fa-warehouse',  'card-info',     'card-icon-info')
        + scard('dead',     'Dead/Retired',    'fas fa-times-circle','card-danger',  'card-icon-danger')
        + scard2('to_check','To Check','fas fa-search',
                 'linear-gradient(135deg,#f0f4ff,#e8effe)',
                 'linear-gradient(135deg,#f59e0b,#fbbf24)','#92400e')
        + scard2('yashwanth','Yashwanth','fas fa-user-shield',
                 'linear-gradient(135deg,#f0fdf4,#dcfce7)',
                 'linear-gradient(135deg,#8b5cf6,#a78bfa)','#5b21b6')
        + scard('users',    'Employees',       'fas fa-users',      'card-warning',  'card-icon-warning')
        + scard('services', 'Service Records', 'fas fa-wrench',     'card-purple',   'card-icon-purple')
        + '</div>'
    )
    ...
    col_l.markdown('<h5 style="color:#1e293b"><i class="fas fa-boxes" style="margin-right:8px"></i>Stock Dashboard</h5>', unsafe_allow_html=True)
    with col_r:
        if st.button("Sync"):
    ...
    if stock_rows:
        ...
        st.info("No stock data. Click Sync to populate.")

    st.markdown('<h5 style="color:#1e293b;margin-top:1rem"><i class="fas fa-clock" style="margin-right:8px"></i>Recently Updated Assets</h5>', unsafe_allow_html=True)
    if recent:
        rows_html = "".join(
            f"<tr><td style='font-weight:700'>{a['asset_no']}</td>"
            f"<td>{a['login_id'] or '-'}</td><td>{a['model'] or '-'}</td>"
            f"<td>{sbadge(a['asset_status']) if a['asset_status'] else '-'}</td>"
            f"<td><span style='font-size:.75rem;color:#64748b'>{a['warranty_status'] or '-'}</span></td></tr>"
            for a in recent)
        st.markdown(
            f'<div class="panel"><div class="panel-body p-0">'
            f'<div class="table-responsive"><table class="table-modern">'
            f'<thead><tr><th>Asset No</th><th>Employee</th><th>Model</th><th>Status</th><th>Warranty</th></tr></thead>'
...
def pg_admin_service_requests():
    st.markdown('<h4 style="color:#1e293b;padding:1.5rem 2rem 0"><i class="fas fa-inbox" style="color:#94a3b8;margin-right:8px"></i>Service Requests</h4>', unsafe_allow_html=True)
    sf = st.selectbox("Filter by Status", ['All', 'Pending', 'Accepted', 'In Progress', 'Hold', 'Completed', 'Declined'])
    conn = get_db()
    ...
    if not reqs:
        ...
        return
    rows_html = "".join(
        f"<tr><td style='font-weight:700;color:#64748b'>#{r['id']}</td>"
        f"<td style='font-weight:600'>{r['employee_name']}</td>"
        f"<td>{r['asset_no'] or '-'}</td>"
        f"<td style='font-weight:600;font-size:.85rem'>{r['service_type']}</td>"
        f"<td>{sbadge(r['status'])}</td>"
        f"<td style='color:#64748b;font-size:.85rem'>{r['assigned_to_name'] or '-'}</td>"
        f"<td style='color:#64748b;font-size:.85rem'>{r['created_at'][:10]}</td></tr>"
        for r in reqs)
    st.markdown(
        f'<div class="panel"><div class="panel-header"><div class="panel-title">'
        f'<i class="fas fa-inbox"></i> Requests ({len(reqs)})</div></div>'
        f'<div class="panel-body p-0"><div class="table-responsive">'
        f'<table class="table-modern"><thead><tr>'
        f'<th>#</th><th>Employee</th><th>Asset</th><th>Issue</th>'
        f'<th>Status</th><th>Assigned</th><th>Date</th></tr></thead>'
        f'<tbody>{rows_html}</tbody></table></div></div></div>',
        unsafe_allow_html=True)
    st.caption("Select a request to manage:")
    cols = st.columns(min(len(reqs), 5))
    for i, r in enumerate(reqs[:10]):
        with cols[i % 5]:
            if st.button(f"#{r['id']} {r['employee_name'][:8]}", key=f"req_{r['id']}", use_container_width=True):
...
def pg_admin_req_detail():
    ...
    st.markdown(f'<h4 style="color:#1e293b;padding:.5rem 0">Request #{req_id} - {req["employee_name"]}</h4>', unsafe_allow_html=True)
    left, right = st.columns([1, 1.6])
    with left:
        st.markdown(
            f'<div class="panel"><div class="panel-header"><div class="panel-title">'
            f'<i class="fas fa-info-circle"></i> Details</div></div>'
            f'<div class="panel-body">'
            f'<div style="text-align:center;margin-bottom:12px">{sbadge(req["status"])}</div>'
            f'<table style="width:100%;font-size:.85rem"><tbody>'
            f'<tr><td style="color:#64748b;font-weight:600;padding:4px 8px">Employee</td><td>{req["employee_name"]}</td></tr>'
            f'<tr><td style="color:#64748b;font-weight:600;padding:4px 8px">Asset</td><td>{req["asset_no"] or "-"}</td></tr>'
            f'<tr><td style="color:#64748b;font-weight:600;padding:4px 8px">Issue</td><td>{req["service_type"]}</td></tr>'
            f'<tr><td style="color:#64748b;font-weight:600;padding:4px 8px">Date</td><td>{req["created_at"][:10]}</td></tr>'
            f'<tr><td style="color:#64748b;font-weight:600;padding:4px 8px">Assigned</td><td>{req["assigned_to_name"] or "-"}</td></tr>'
            f'</tbody></table>'
            f'<div style="margin-top:12px;background:#f8fafc;border-radius:8px;padding:10px;font-size:.85rem">'
            f'{req["description"]}</div></div></div>',
            unsafe_allow_html=True)
        ...
        with st.form("sf"):
            ...
            ns = st.selectbox("Change Status", opts, index=opts.index(req['status']))
            if st.form_submit_button("Update", use_container_width=True): _set_status(req_id, req, ns); st.rerun()
        if techs:
            with st.form("tf"):
    ...
    with right:
        ...
        for m in msgs:
            ...
            align = 'flex-end' if is_adm else 'flex-start'
            tr_end = '12px 12px 0 12px' if is_adm else '12px 12px 12px 0'
            lbl = f'You (Admin)' if is_adm else f'{m["sender_name"]} ({m["sender_role"]})'
            msgs_html += (
                f'<div style="display:flex;justify-content:{align};margin-bottom:12px">'
                f'<div style="max-width:75%">'
                f'<div style="font-size:.75rem;color:#94a3b8;margin-bottom:4px;text-align:{"right" if is_adm else "left"}">'
                f'{lbl} · {m["created_at"][11:16]}</div>'
                f'<div style="padding:10px 14px;word-break:break-word;background:{bg};color:{tc};border-radius:{tr_end}">'
                f'{m["message"]}</div></div></div>')
        st.markdown(
            f'<div class="panel" style="min-height:500px;display:flex;flex-direction:column">'
            f'<div class="panel-header"><div class="panel-title"><i class="fas fa-comments"></i> Chat</div></div>'
            f'<div style="flex:1;overflow-y:auto;padding:20px;background:#f8fafc;min-height:350px">'
            f'{msgs_html or "<div style=text-align:center;padding:40px;color:#94a3b8><i class=fas fa-comments style=font-size:2rem;display:block;margin-bottom:8px></i>No messages yet</div>"}'
            f'</div></div>',
            unsafe_allow_html=True)
        with st.form("cf", clear_on_submit=True):
            mi = st.text_area("", placeholder="Type message...", height=80, label_visibility="collapsed")
...
def _assign(req_id, req, tech_id, tech_name):
    ...
    if tech and tech['email'] and nc['notify_technician_assigned']:
        send_email([tech['email']], f"Request #{req_id} Assigned to You", f"<p>Request #{req_id} has been assigned to you.</p>")
...
def pg_admin_assets():
    st.markdown('<h4 style="color:#1e293b;padding:1.5rem 2rem 0"><i class="fas fa-laptop" style="color:#94a3b8;margin-right:8px"></i>All Assets</h4>', unsafe_allow_html=True)
    c1, c2 = st.columns([3, 1])
    search = c1.text_input("", placeholder="Search employee, model, asset...", label_visibility="collapsed")
    sf = c2.selectbox("", ['All', 'Assigned', 'IT Stock', 'DEAD', 'To Check', 'Yashwanth'], label_visibility="collapsed")
    conn = get_db(); q = "SELECT * FROM assets WHERE 1=1"; p = []
    ...
    if assets:
        rows_html = "".join(
            ...
            f"<td>{a['login_id'] or '-'}</td><td>{a['model'] or '-'}</td>"
            f"<td>{sbadge(a['asset_status']) if a['asset_status'] else '-'}</td>"
            f"<td style='font-size:.8rem;color:#64748b'>{a['serial_no'] or '-'}</td></tr>"
            for a in assets)
        st.markdown(
            f'<div class="panel"><div class="panel-header"><div class="panel-title">'
            f'<i class="fas fa-laptop"></i> Assets ({len(assets)})</div></div>'
            f'<div class="panel-body p-0"><div class="table-responsive"><table class="table-modern">'
            f'<thead><tr><th>Asset No</th><th>Employee</th><th>Model</th><th>Status</th><th>Serial</th></tr></thead>'
            f'<tbody>{rows_html}</tbody></table></div></div></div>',
            unsafe_allow_html=True)
    else: st.info("No assets found.")

def pg_admin_employees():
    st.markdown('<h4 style="color:#1e293b;padding:1.5rem 2rem 0"><i class="fas fa-users" style="color:#94a3b8;margin-right:8px"></i>Employees</h4>', unsafe_allow_html=True)
    conn = get_db()
    ...
    rows_html = "".join(
        f"<tr><td style='font-weight:600'>{e['login_id']}</td>"
        f"<td>{e['cnt']}</td><td style='font-size:.8rem;color:#64748b'>{e['models'] or '-'}</td></tr>"
        for e in emps if not search or search.lower() in e['login_id'].lower())
    st.markdown(
        f'<div class="panel"><div class="panel-body p-0"><div class="table-responsive">'
        f'<table class="table-modern"><thead><tr><th>Employee</th><th>Assets</th><th>Models</th></tr></thead>'
        f'<tbody>{rows_html or "<tr><td colspan=3 style=text-align:center;padding:32px;color:#94a3b8>No employees found</td></tr>"}</tbody>'
        f'</table></div></div></div>',
        unsafe_allow_html=True)

def pg_admin_services():
    st.markdown('<h4 style="color:#1e293b;padding:1.5rem 2rem 0"><i class="fas fa-tools" style="color:#94a3b8;margin-right:8px"></i>Service History</h4>', unsafe_allow_html=True)
    search = st.text_input("", placeholder="Search...", label_visibility="collapsed")
    conn = get_db(); q = "SELECT sh.*,a.login_id FROM service_history sh LEFT JOIN assets a ON sh.asset_no=a.asset_no WHERE 1=1"; p = []
    ...
    rows_html = "".join(
        f"<tr><td style='font-size:.8rem'>{s['service_date'] or '-'}</td>"
        f"<td style='font-weight:600'>{s['asset_no']}</td><td>{s['login_id'] or '-'}</td>"
        f"<td>{s['service_type'] or '-'}</td><td>{sbadge(s['status'])}</td>"
        f"<td>{s['technician'] or '-'}</td></tr>"
        for s in svcs)
    st.markdown(
        f'<div class="panel"><div class="panel-header"><div class="panel-title">'
        f'<i class="fas fa-tools"></i> Service Records ({len(svcs)})</div></div>'
        f'<div class="panel-body p-0"><div class="table-responsive"><table class="table-modern">'
        f'<thead><tr><th>Date</th><th>Asset</th><th>Employee</th><th>Type</th><th>Status</th><th>Tech</th></tr></thead>'
        f'<tbody>{rows_html or "<tr><td colspan=6 style=text-align:center;padding:32px;color:#94a3b8>No records</td></tr>"}</tbody>'
        f'</table></div></div></div>',
        unsafe_allow_html=True)

def pg_admin_users():
    st.markdown('<h4 style="color:#1e293b;padding:1.5rem 2rem 0"><i class="fas fa-user-cog" style="color:#94a3b8;margin-right:8px"></i>Manage Users</h4>', unsafe_allow_html=True)
    conn = get_db(); users = conn.execute("SELECT * FROM users ORDER BY role,username").fetchall(); conn.close()
    role_badge = lambda r: sbadge(r.upper())
    rows_html = "".join(
        f"<tr><td>{u['id']}</td><td style='font-weight:600'>{u['username']}</td>"
        f"<td>{u['employee_name'] or '-'}</td><td>{role_badge(u['role'])}</td>"
        f"<td style='font-size:.8rem'>{u['email'] or '-'}</td></tr>"
        for u in users)
    st.markdown(
        f'<div class="panel"><div class="panel-header"><div class="panel-title">'
        f'<i class="fas fa-users"></i> All Users ({len(users)})</div></div>'
        f'<div class="panel-body p-0"><div class="table-responsive"><table class="table-modern">'
        f'<thead><tr><th>ID</th><th>Username</th><th>Name</th><th>Role</th><th>Email</th></tr></thead>'
        f'<tbody>{rows_html}</tbody></table></div></div></div>',
        unsafe_allow_html=True)
    st.markdown('<div class="panel" style="margin-top:12px"><div class="panel-header"><div class="panel-title"><i class="fas fa-user-plus"></i> Add User</div></div><div class="panel-body">', unsafe_allow_html=True)
    with st.form("auf"):
...
def pg_email_config():
    st.markdown('<h4 style="color:#1e293b;padding:1.5rem 2rem 0"><i class="fas fa-cog" style="color:#94a3b8;margin-right:8px"></i>Email Configuration</h4>', unsafe_allow_html=True)
    cfg = configparser.ConfigParser(); cfg.read(CONFIG_PATH)
    smtp = dict(cfg['SMTP']) if 'SMTP' in cfg else {}
    notif = dict(cfg['NOTIFICATIONS']) if 'NOTIFICATIONS' in cfg else {}
    t1, t2, t3 = st.tabs(["SMTP Settings", "Notifications", "Test Email"])
    with t1:
        st.success(f"Configured: {smtp.get('smtp_user')}") if smtp.get('smtp_user') else st.warning("Not configured yet.")
        with st.form("smf"):
            c1, c2 = st.columns([3, 1])
            h = c1.text_input("SMTP Host", value=smtp.get('smtp_host', 'smtp.gmail.com'))
            p = c2.text_input("Port", value=smtp.get('smtp_port', '587'))
            ...
            if st.form_submit_button("Save SMTP", use_container_width=True):
                if 'SMTP' not in cfg: cfg['SMTP'] = {}
                cfg['SMTP'].update({'SMTP_HOST': h, 'SMTP_PORT': p, 'SMTP_USER': u, 'SMTP_PASSWORD': pw, 'FROM_NAME': fn})
                with open(CONFIG_PATH, 'w') as f: cfg.write(f)
                st.success("Saved!"); st.rerun()
    with t2:
        def nb(k): return notif.get(k, 'true') != 'false'
        with st.form("nf"):
            n1 = st.checkbox("New request - notify admins", value=nb('notify_new_request'))
            ...
            if st.form_submit_button("Save Preferences", use_container_width=True):
                ...
                for k, v in zip(['notify_new_request', 'notify_employee_action', 'notify_technician_assigned',
                                  'notify_status_employee', 'notify_status_admin', 'notify_chat'], [n1, n2, n3, n4, n5, n6]):
...
def pg_user_dashboard():
    ...
    conn.close()
    st.markdown(
        f'<div style="margin:1.5rem 2rem;background:linear-gradient(135deg,#0f3460,#533483);'
        f'color:#fff;padding:24px 28px;border-radius:16px">'
        f'<h4 style="margin:0;color:#fff">Welcome back, {name}!</h4>'
        f'<p style="margin:6px 0 0;opacity:.8">Here is your IT asset overview</p></div>',
        unsafe_allow_html=True)
    ...
    if assets:
        rows_html = "".join(
            ...
            for a in assets)
        st.markdown(
            f'<div class="panel" style="margin:1rem 2rem"><div class="panel-header">'
            f'<div class="panel-title"><i class="fas fa-laptop"></i> My Assets</div></div>'
            f'<div class="panel-body p-0"><div class="table-responsive"><table class="table-modern">'
            f'<thead><tr><th>Asset No</th><th>Model</th><th>Status</th><th>Serial</th></tr></thead>'
            f'<tbody>{rows_html}</tbody></table></div></div></div>',
            unsafe_allow_html=True)
    else: st.info("No assets assigned to you.")

def pg_user_my_requests():
    st.markdown('<h4 style="color:#1e293b;padding:1.5rem 2rem 0"><i class="fas fa-ticket-alt" style="color:#94a3b8;margin-right:8px"></i>My Service Requests</h4>', unsafe_allow_html=True)
    ...
    rows_html = "".join(
        f"<tr><td style='font-weight:700;color:#64748b'>#{r['id']}</td>"
        f"<td style='font-weight:600'>{r['service_type']}</td>"
        f"<td>{r['asset_no'] or '-'}</td><td>{sbadge(r['status'])}</td>"
        f"<td style='font-size:.8rem;color:#64748b'>{r['assigned_to_name'] or '-'}</td>"
        f"<td style='font-size:.8rem;color:#64748b'>{r['created_at'][:10]}</td></tr>"
        for r in reqs)
    st.markdown(
        f'<div class="panel" style="margin:1rem 2rem"><div class="panel-header">'
        f'<div class="panel-title"><i class="fas fa-list"></i> Requests ({len(reqs)})</div></div>'
        f'<div class="panel-body p-0"><div class="table-responsive"><table class="table-modern">'
        f'<thead><tr><th>#</th><th>Issue</th><th>Asset</th><th>Status</th><th>Assigned</th><th>Date</th></tr></thead>'
        f'<tbody>{rows_html}</tbody></table></div></div></div>',
        unsafe_allow_html=True)
    st.caption("Click to open a request:")
    cols = st.columns(min(len(reqs), 4))
    for i, r in enumerate(reqs[:8]):
        with cols[i % 4]:
            if st.button(f"#{r['id']} View", key=f"ur_{r['id']}", use_container_width=True):
                st.session_state.req_id = r['id']; st.session_state.page = 'req_detail_user'; st.rerun()

def _render_chat_panel(req, msgs, sender_role, submit_url):
    msgs_html = ""
    for m in msgs:
        ...
        tc = '#fff' if m['sender_role'] in ('admin', 'technician') or is_me else '#1e293b'
        align = 'flex-end' if is_me else 'flex-start'
        tr = '12px 12px 0 12px' if is_me else '12px 12px 12px 0'
        lbl = 'You' if is_me else f'{m["sender_name"]} ({m["sender_role"]})'
        msgs_html += (
            f'<div style="display:flex;justify-content:{align};margin-bottom:12px">'
            f'<div style="max-width:75%">'
            f'<div style="font-size:.75rem;color:#94a3b8;margin-bottom:4px;text-align:{"right" if is_me else "left"}">'
            f'{lbl} · {m["created_at"][11:16]}</div>'
            f'<div style="padding:10px 14px;word-break:break-word;background:{bg};color:{tc};border-radius:{tr}">'
            f'{m["message"]}</div></div></div>')
    st.markdown(
        f'<div class="panel" style="min-height:460px;display:flex;flex-direction:column">'
        f'<div class="panel-header"><div class="panel-title"><i class="fas fa-comments"></i> Chat</div></div>'
        f'<div style="flex:1;overflow-y:auto;padding:20px;background:#f8fafc;min-height:300px">'
        f'{msgs_html or "<div style=text-align:center;padding:40px;color:#94a3b8><i class=fas fa-comments style=font-size:2rem;display:block;margin-bottom:8px></i>No messages yet</div>"}'
        f'</div></div>',
        unsafe_allow_html=True)
    if req['status'] not in ('Declined', 'Completed'):
        with st.form("chat_" + sender_role, clear_on_submit=True):
            mi = st.text_area("", placeholder="Type your message...", height=80, label_visibility="collapsed")
            if st.form_submit_button("Send Message", use_container_width=True):
                if mi.strip(): _chat(req['id'], req, mi.strip(), sender_role); st.rerun()
    else:
        st.info(f"Chat closed - request is {req['status'].lower()}.")
...
def pg_user_req_detail():
    ...
    msgs = conn.execute("SELECT * FROM chat_messages WHERE request_id=? ORDER BY created_at", (req_id,)).fetchall()
    conn.close()
    if st.button("Back to My Requests"): st.session_state.page = 'my_requests'; st.rerun()
    st.markdown(f'<h4 style="color:#1e293b;padding:.5rem 0">Request #{req_id}</h4>', unsafe_allow_html=True)
    left, right = st.columns([1, 1.6])
    with left:
        st.markdown(
            f'<div class="panel"><div class="panel-header"><div class="panel-title"><i class="fas fa-info-circle"></i> Details</div></div>'
            f'<div class="panel-body"><div style="text-align:center;margin-bottom:12px">{sbadge(req["status"])}</div>'
            f'<table style="width:100%;font-size:.85rem"><tbody>'
            f'<tr><td style="color:#64748b;font-weight:600;padding:4px 8px">Asset</td><td>{req["asset_no"] or "-"}</td></tr>'
            f'<tr><td style="color:#64748b;font-weight:600;padding:4px 8px">Issue</td><td>{req["service_type"]}</td></tr>'
            f'<tr><td style="color:#64748b;font-weight:600;padding:4px 8px">Submitted</td><td>{req["created_at"][:10]}</td></tr>'
            f'<tr><td style="color:#64748b;font-weight:600;padding:4px 8px">Assigned To</td><td>{req["assigned_to_name"] or "Not assigned yet"}</td></tr>'
            f'</tbody></table>'
            f'<div style="margin-top:12px;background:#f8fafc;border-radius:8px;padding:10px;font-size:.85rem">{req["description"]}</div>'
            f'</div></div>',
            unsafe_allow_html=True)
    with right:
        _render_chat_panel(req, msgs, 'user', '')

def pg_request_service():
    st.markdown('<h4 style="color:#1e293b;padding:1.5rem 2rem 0"><i class="fas fa-wrench" style="color:#94a3b8;margin-right:8px"></i>Submit Service Request</h4>', unsafe_allow_html=True)
    name = st.session_state.employee_name
    conn = get_db()
...
def pg_email_prefs():
    ...
    st.markdown('<h4 style="color:#1e293b;padding:1.5rem 2rem 0"><i class="fas fa-bell" style="color:#94a3b8;margin-right:8px"></i>Email Preferences</h4>', unsafe_allow_html=True)
    if uemail: st.info(f"Notifications will be sent to: {uemail}")
    else: st.warning("No email set on your account. Ask your admin to add one.")
    with st.form("epf"):
        if role == 'technician':
            ...
            if st.form_submit_button("Save Preferences", use_container_width=True):
                np = {'assigned': na, 'chat': nc, 'status': ns}
                conn2 = get_db(); conn2.execute("UPDATE users SET notification_prefs=? WHERE id=?", (json.dumps(np), uid)); conn2.commit(); conn2.close(); st.success("Saved!")
        else:
            ...
            if st.form_submit_button("Save Preferences", use_container_width=True):
                np = {'action': na, 'status': ns, 'chat': nc}
                conn2 = get_db(); conn2.execute("UPDATE users SET notification_prefs=? WHERE id=?", (json.dumps(np), uid)); conn2.commit(); conn2.close(); st.success("Saved!")
...
def pg_tech_dashboard():
    ...
    conn.close()
    st.markdown('<h4 style="color:#1e293b;padding:1.5rem 2rem 0"><i class="fas fa-tachometer-alt" style="color:#94a3b8;margin-right:8px"></i>Technician Dashboard</h4>', unsafe_allow_html=True)
    ...
    rows_html = "".join(
        f"<tr><td style='font-weight:700;color:#64748b'>#{r['id']}</td>"
        f"<td style='font-weight:600'>{r['employee_name']}</td>"
        f"<td>{r['asset_no'] or '-'}</td><td style='font-size:.85rem'>{r['service_type']}</td>"
        f"<td>{sbadge(r['status'])}</td><td style='font-size:.8rem;color:#64748b'>{r['updated_at'][:10]}</td></tr>"
        for r in reqs)
    st.markdown(
        f'<div class="panel" style="margin:1rem 2rem"><div class="panel-header">'
        f'<div class="panel-title"><i class="fas fa-tasks"></i> Assigned Requests</div></div>'
        f'<div class="panel-body p-0"><div class="table-responsive"><table class="table-modern">'
        f'<thead><tr><th>#</th><th>Employee</th><th>Asset</th><th>Issue</th><th>Status</th><th>Updated</th></tr></thead>'
        f'<tbody>{rows_html}</tbody></table></div></div></div>',
        unsafe_allow_html=True)
    st.caption("Click to open a request:")
    cols = st.columns(min(len(reqs), 4))
    for i, r in enumerate(reqs[:8]):
        with cols[i % 4]:
            if st.button(f"#{r['id']} Open", key=f"tr_{r['id']}", use_container_width=True):
    ...
    with left:
        ...
        if req['status'] != 'Completed':
            with st.form("tsf"):
                ...
                ns = st.selectbox("Update Status", opts, index=opts.index(req['status']) if req['status'] in opts else 0)
                if st.form_submit_button("Update", use_container_width=True):
                    ...
                    st.success(f"Status updated to {ns}!"); st.rerun()
        else: st.success("Task completed.")
    with right:
        _render_chat_panel(req, msgs, 'technician', '')
